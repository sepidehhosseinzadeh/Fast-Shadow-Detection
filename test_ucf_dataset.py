...
# RGB image
TrainImgeFolder = '/home/sepideh/Documents/illuminChngeLrning/data/UCF/355/train/img/';
# mask
TrainMaskFolder = '/home/sepideh/Documents/illuminChngeLrning/data/UCF/355/train/gt/';
# paired reg map (image-level prior)
TrainFCNFolder = '/home/sepideh/Documents/illuminChngeLrning/data/UCF/355/train/pairedr/';

# RGB image
TestImgeFolder = '/home/sepideh/Documents/illuminChngeLrning/data/UCF/355/test/img/';
# mask
TestMaskFolder = '/home/sepideh/Documents/illuminChngeLrning/data/UCF/355/test/gt/';
# paired reg map (image-level prior)
TestFCNFolder = '/home/sepideh/Documents/illuminChngeLrning/data/UCF/355/test/pairedr/';
...
def cnn_pred_mask(cnn, Test):
    ...
    for im_id in range(len(X)):
        ...
        # pdb.set_trace()
        misc.imsave('./' + OutputFolder + '/' + F[im_id][:-4] + '_putMiddlePoint.png', pred_m[16:rowSize - 16, 16:colSize - 16]);
...
def load_data(image_folder, mask_folder, fcn_folder):
    ...
    for fn in os.listdir(image_folder):
        if os.path.isfile(image_folder + fn):
            if not os.path.exists(mask_folder + fn[:-4] + '.png'):
                continue;
            ...
            im_meaned[:, :, 0] = (im[:, :, 0].astype(np.float32) - 129.41) / 100.0;
            im_meaned[:, :, 1] = (im[:, :, 1].astype(np.float32) - 128.10) / 100.0;
            im_meaned[:, :, 2] = (im[:, :, 2].astype(np.float32) - 120.69) / 100.0;

            fcn = misc.imread(fcn_folder + fn[:-4] + '.png').astype(np.float32);
            im_meaned[:, :, 3] = (fcn - 95.11) / 100.0;

            edge = canny_edge(im);
            mask = misc.imread(mask_folder + fn[:-4] + '.png');
