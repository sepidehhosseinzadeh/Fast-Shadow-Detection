...
def cnn_pred_mask(cnn, Test):
    ...
    for im_id in range(len(X)):
        ...
        '''predict_patches = cnn.predict(net_inputs).reshape((-1, 32, 32));  # 1x32x32

        for patch_idx in range(0, len(predict_patches)):
            i, j = indices[patch_idx];
            if (check[i, j] == 1):
                continue;
            ...
            if (pred_m[i, j] != pred_m[i_d, j_d] or pred_m[i, j] != pred_m[i_r, j_r]):
                predict_patch = predict_patches[patch_idx];

                sum_neigh = 0.0;
                for d in range(9):
                    sum_neigh += predict_patch[32 / 2 + x_d[d], 32 / 2 + y_d[d]];
                mean_neigh = sum_neigh / 9.0;
