...
def cnn_pred_mask(cnn, Test):
    ...
    for im_id in range(len(X)):
        ...
        scale = 1;
        row, col = 1, 1;
        rrow, ccol = rowSize - 2, colSize - 2;

        t_while = time.time();
        t_total_only_cnn_for_while = 0;
        ...
        while (True):
            ...
            if (row <= 17):
                for j in range(1, colSize - 1):
                    ...
                    if (pred_m[row, j] != pred_m[i_d, j_d] or pred_m[row, j] != pred_m[i_r, j_r]):
                        net_input = img[scale * row - 1: scale * row + 32 - 1,
                                    scale * j - 1:  scale * j + 32 - 1, :].transpose();
                        s_i, s_j = row - 1, j - 1;
                        if (row - s_i > 30 or j - s_j > 30 or net_input.shape != (4, 32, 32)):
                            continue;
                        net_inputs.append(net_input);
                        indices.append([row, j]);
                        indices_patch.append([row - s_i, j - s_j]);
                # endfor_j
                row += 1;
            ...
            if (col <= 17):
                for i in range(rowSize - 2, -1, -1):
                    ...
                    if (pred_m[i, col] < threshold and pred_m[i_u, j_u] < threshold and pred_m[i_r, j_r] < threshold):
                        continue;

                    if (pred_m[i, col] != pred_m[i_u, j_u] or pred_m[i, col] != pred_m[i_r, j_r]):
                        net_input = img[scale * i - 1:  scale * i + 32 - 1, scale * col - 1: scale * col + 32 - 1,
                                    :].transpose();
                        s_i, s_j = i - 1, col - 1;
                        if (i - s_i > 30 or col - s_j > 30 or net_input.shape != (4, 32, 32)):
                            continue;
                        net_inputs.append(net_input);
                        indices.append([i, col]);
                        indices_patch.append([i - s_i, col - s_j]);
                # endfor_i
                col += 1;
            ...
            if (row <= rowSize - 17):
                for j in range(colSize - 2, -1, -1):
                    ...
                    i_l = rrow;
                    j_l = max(j - 1, 0);
                    if (pred_m[rrow, j] < threshold and pred_m[i_u, j_u] < threshold and pred_m[i_l, j_l] < threshold):
                        continue;

                    if (pred_m[rrow, j] != pred_m[i_u, j_u] or pred_m[rrow, j] != pred_m[i_l, j_l]):
                        net_input = img[scale * rrow - 32 + 2: scale * rrow + 2,
                                    scale * j - 32 + 2:  scale * j + 2, :].transpose();
                        s_i, s_j = rrow - 32 + 2, j - 32 + 2;
                        if (rrow - s_i > 30 or j - s_j > 30 or net_input.shape != (4, 32, 32)):
                            continue;
                        net_inputs.append(net_input);
                        indices.append([rrow, j]);
                        indices_patch.append([rrow - s_i, j - s_j]);
                # endfor_j
                rrow -= 1;
            ...
            if (col <= colSize - 17):
                for i in range(1, rowSize - 1):
                    ...
                    i_d = max(i + 1, rowSize - 1);
                    j_d = ccol;
                    i_l = i;
                    j_l = max(ccol - 1, 0);
                    if (pred_m[i, ccol] < threshold and pred_m[i_d, j_d] < threshold and pred_m[i_l, j_l] < threshold):
                        continue;

                    if (pred_m[i, ccol] != pred_m[i_d, j_d] or pred_m[i, ccol] != pred_m[i_l, j_l]):
                        net_input = img[scale * i - 32 + 2:  scale * i + 2, scale * ccol - 32 + 2: scale * ccol + 2,
                                    :].transpose();
                        s_i, s_j = i - 32 + 2, ccol - 32 + 2;
                        if (i - s_i > 30 or ccol - s_j > 30 or net_input.shape != (4, 32, 32)):
                            continue;
                        net_inputs.append(net_input);
                        indices.append([i, ccol]);
                        indices_patch.append([i - s_i, ccol - s_j]);
                # endfor_i
                ccol -= 1;
            ...
            t_total_only_cnn_for_while += (time.time() - t_cnn_2);

            for p in range(0, len(predict_patches)):
                ii, jj = indices[p];
                i_p, j_p = indices_patch[p];
                sum_neigh = 0.0;
                for d in range(9):
                    sum_neigh += predict_patches[p, i_p + x_d[d], j_p + y_d[d]];
